...
3. The limitations of measurement and observation.
  - E.g., our "continuous" attitude may not be observable in any fashion expect an ordinal variable.
...
**Data-generating processes lead to empirical distributions, which map onto random variables distributed by functions with known properties and assumptions.**
...
* Taking the stochastic component seriously, upfront, is central to accomplishing this.
...
We can then formulate a stastical model of the DGP as follows:
...
* We can use any distribution that approximates are dependent variable.
...
* Note that we tackle the stochastic component of the dependent variable upfront - before we attempt to model it (impose our theory).
...
* Choosing a new distribution, $\mathcal{f}(\cdot)$, would give us a new model, even though our systematic component would be the same. This is not possible in the error-term approach.

---

## Writing and Talking about Models

So, in a paper, we may write are model, assuming a Gaussian dependent variable, such that
\begin{align}
y &\sim \mathcal{f}_N(\mu, \sigma^2) \\
\mu &= X\beta
\end{align}
